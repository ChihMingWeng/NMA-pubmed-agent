...
with st.sidebar:
    ...
    template_name = st.selectbox("Disease template", list(DISEASE_TEMPLATES), index=list(DISEASE_TEMPLATES).index("Severe Asthma Biologics"))
    selected_template = DISEASE_TEMPLATES.get(template_name, DEFAULT_TOPIC)
    st.write(f"Loaded template: {template_name}. You can edit all fields before searching.")
    condition = st.text_input("Condition / population", value=selected_template["condition"])
    interventions = st.text_area("Interventions (comma, semicolon, or newline separated)", value=selected_template["interventions"])
    comparators = st.text_area("Comparators / placebo / standard care", value=selected_template["comparators"])
    outcome = st.text_area("Outcome(s)", value=selected_template["outcome"])
    study_design = st.radio("Study design", STUDY_DESIGNS, index=0)
...
if query:
    final_pubmed_url = pubmed_search_url(query)
    st.text_input("Final encoded PubMed URL", value=final_pubmed_url)
    st.link_button("Open this query in PubMed", final_pubmed_url)
...
st.subheader("6. Exportable NMA dataset")
nma_dataset = nma_dataset_from_edges(edges)
st.dataframe(nma_dataset, hide_index=True, use_container_width=True)
st.download_button(
    "Download NMA dataset as CSV",
    nma_dataset.to_csv(index=False).encode("utf-8"),
    file_name="nma_dataset.csv",
    mime="text/csv",
)
st.download_button(
    "Download NMA dataset as TIFF",
    dataframe_to_tiff(nma_dataset, title="NMA dataset"),
    file_name="nma_dataset.tif",
    mime="image/tiff",
)

st.subheader("7. NMA protocol checklist")
